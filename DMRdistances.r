...
greppata <- grep("LTR|LINE|DNA|RC", extractA$Feature)
extractA$Feature[greppata] <- "TE"
greppata <- grep("LTR|LINE|DNA|RC", extractB$Feature)
extractB$Feature[greppata] <- "TE"
greppata <- grep("LTR|LINE|DNA|RC", extractC$Feature)
extractC$Feature[greppata] <- "TE"
...
ds <- data.frame(Feature = c(extractA$Feature,extractB$Feature,extractC$Feature),
...
# Divide it by 1000 to obtain kbp:
ds$Dist <- ds$Dist / 1000

# Plot:
xp <- c(0, 0.8, 1.6, 2.4, 3.2, 5.6, 6.4, 7.2, 8, 8.8, 11.2, 12, 12.8, 13.6, 14.4)
colvec <- rep(c("aquamarine3","cadetblue3","chartreuse3","darkorchid3",
                "deeppink3"), 3)
xnames <- c("CDS","INT","PRO","REP","TE")

boxw <- boxplot(Dist ~ Feature + Comp, data = ds, outline = FALSE, at = xp,
                ylab = "Distance to DMR (kbp)", notch = FALSE,
                axes = FALSE, frame.plot = TRUE, main = "",
                col = colvec,
                ylim = c(20, 795), #par(mar = par("mar")),
...
text(c(1.6, 7.2, 12.8), 780, labels = c("S","I","M"), cex = 1.3)
