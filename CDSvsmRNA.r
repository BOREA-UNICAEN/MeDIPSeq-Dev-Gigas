...
methCDSDevAll<-data.frame(methCDS, testRes)


# Exclude all genes rows for which ANOVA gave non-significant results:
df<-methCDSDevAll[!is.na(methCDSDevAll$testRes)& methCDSDevAll$testRes< 0.01,] 
...
methCDSByStage<-data.frame(df[,1],
									(df[,15]+df[,16])/2,#ovo
									(df[,2]+df[,3]+df[,4])/3,#28
									(df[,13]+df[,14])/2,#mor
									(df[,5]+df[,6])/2,#bla
...
colnames(methCDSByStage)<-c("GeneID","ovo","x28","mor","bla","gas","tro","Dla","spa")


#jointure methCDS et expression
cds.exp<-merge (methCDSByStage, mRNAByStage, by="GeneID")
cds.exp<-as.data.frame(cds.exp)
#filtrage expression nulle
cds.exp<-cds.exp[cds.exp$expmoydev!=0,]
...
#correlation
for (i in 1:nrow(cds.exp))
{
#correlation[,1]<-exp.met
y<-as.numeric(t(cds.exp[i,2:9]))
x<-as.numeric(t(cds.exp[i,10:17]))
lm.x.y<-lm(y~x)
...
plot(x=cds.exp[id.significatif,]$ovo, y=cds.exp[id.significatif,]$expov)
#save.image("evsgbMsovo.jpg")
plot(x=cds.exp[id.significatif,]$x28, y=cds.exp[id.significatif,]$exp2.8cells)
plot(x=cds.exp[id.significatif,]$mor, y=cds.exp[id.significatif,]$expmo)
plot(x=cds.exp[id.significatif,]$bla, y=cds.exp[id.significatif,]$expbl)
plot(x=cds.exp[id.significatif,]$gas, y=cds.exp[id.significatif,]$expga)
plot(x=cds.exp[id.significatif,]$tro, y=cds.exp[id.significatif,]$exptr)
plot(x=cds.exp[id.significatif,]$Dla, y=cds.exp[id.significatif,]$expdl)
plot(x=cds.exp[id.significatif,]$spa, y=cds.exp[id.significatif,]$expsp)
